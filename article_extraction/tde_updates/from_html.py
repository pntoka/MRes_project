...
def makearray_xml(xml_table):
    """
    Creates numpy array from xml table
    """
    n_cols = 0
    n_rows = 0

    for row in xml_table.findAll('row'):
        col_tags = row.find_all('entry')
        if len(col_tags) > 0:
            n_rows += 1
            if len(col_tags) > n_cols:
                n_cols = len(col_tags)
    ...
    for row in xml_table.findAll('row'):
        ...
        if len(row.find_all('entry')) == 0:
            continue

        else:

            columns = row.find_all('entry')
            col_dim = []
            row_dim = []
            col_dim_counter = -1
            row_dim_counter = -1
...
def read_file(file_path, table_number=1):
    ...
    doc = file.read()
    if doc.startswith('<component xmlns') or doc.startswith('<full-text-retrieval-response xmlns'):
        xml_soup = BeautifulSoup(doc, features='xml')
        file.close()
        xml_table = xml_soup.find_all("table")[table_number-1]
